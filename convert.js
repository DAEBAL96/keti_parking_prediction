...
let cnt_url = "http://203.253.128.164:7579/Mobius/AIparking"
// let cnt_url = cse_url + "/" + m2m_ae["Deep"] + "/" + Deep_spot_list[deep_spot_count] + "?rcn=4&ty=4&cra=" + parsing_start_day
...
const make_parking_csv = async function(){
    try{
        ...
        for(var deep_spot_count = 0; deep_spot_count < Deep_spot_list.length; deep_spot_count++){
            ...
            if(Object.keys(res.data["m2m:rsp"]).length === 0){     // parsing_start_day 이후로 적측된 cin이 없음
                console.log("**", Deep_spot_list[deep_spot_count], "** spot's date state not exist")
            }

            else{
                for(var i=0; i < res.data["m2m:rsp"]["m2m:cin"].length; i++){
                    AIparking_state[res.data["m2m:rsp"]["m2m:cin"][i]["ct"]] = res.data["m2m:rsp"]["m2m:cin"][i]["con"];
                }
        ...
        for(var LoRa_spot_count = 0; LoRa_spot_count < LoRa_spot_list.length; LoRa_spot_count++){
            ...
            if(Object.keys(res.data["m2m:rsp"]).length === 0){
                console.log("**",parking_conf_json["LoRa"][LoRa_spot_list[LoRa_spot_count]], "** spot's date state not exist")
            }

            else{
                for(var i=0; i < res.data["m2m:rsp"]["m2m:cin"].length; i++){
                    AIparking_state[res.data["m2m:rsp"]["m2m:cin"][i]["ct"]] = res.data["m2m:rsp"]["m2m:cin"][i]["con"]["parking"];
                }
