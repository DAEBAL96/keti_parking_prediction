...
const make_init_parking_state = async function(){
    try{
        ...
        for(var deep_spot_count = 0; deep_spot_count < Deep_spot_list.length; deep_spot_count++){
            ...
            let db_res = JSON.parse(fs.readFileSync(`./2023/${Deep_spot_list[deep_spot_count]}.json`, 'utf8'));

            if(db_res.length === 0){     // parsing_start_day 이후로 적측된 cin이 없음
                console.log("**", Deep_spot_list[deep_spot_count], "** spot's date state not exist")
            }

            else{
                var AIparking_state = {};
                var cin_length = db_res.length
                if(common_min_time === null){
                    common_min_time = Number(db_res[0]['ct'].replace('T',''));
                }
                else if(common_min_time < Number(db_res[0]['ct'].replace('T',''))){
                    common_min_time = Number(db_res[0]['ct'].replace('T',''));
                }

                if(csv_latest_time === null){
                    csv_latest_time = Number(db_res[cin_length-1]['ct'].replace('T',''));
                }
                else if(csv_latest_time < Number(db_res[cin_length-1]['ct'].replace('T',''))){
                    csv_latest_time = Number(db_res[cin_length-1]['ct'].replace('T',''));
                }
                ...
                for(var i=0; i < db_res.length; i++){
                    AIparking_state[db_res[i]["ct"]] = db_res[i]["con"];
                }
